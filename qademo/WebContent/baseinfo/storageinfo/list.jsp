...
<!-- 
	...
	function add(){
		window.location.href = "storageinfo!addQuery.action";
	}
	function callback(msg){
		
		if(msg==0){
		document.listForm.action ="storageinfo!remove.action";
		document.listForm.submit();
		saveOkTip('删除中，请稍候....' , 'btn' , 'delBtn');
		}else if(msg==2){
			alert("请先撤销库房与库工的关联，您可以进入【库房编辑】页面完成这项工作!");
		}else if(msg==1){
			alert("请先撤销该库房与保管员的关联，您可以通过了在【保管员编辑】页面完成这项操作!");
		}else if(msg==3){
			alert("请先撤销该库房与库位信息的关联，您可以通过了在【库位信息编辑】页面完成这项操作!");
		}
	}
	...
	function remov(){
		...
		if(confirm("您确定要删除选中的记录吗？")==true){
			...
			for(var i=0;i<myids.length;i++){
				if(myids[i].checked){
					temparr.push(myids[i].value);
				}			
				
			}
			storageinfoService.isRemove(temparr,callback);
		}
	}	
...
<form name="listForm" action="" method="post">
	...
	<td width="10%" align="center">库房编号</td>
	<td width="10%" align="center">库房名称</td>
	<td width="20%" align="center">库工</td>
	<td width="10%" align="center">保管员</td>
